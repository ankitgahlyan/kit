...
export class EventRouter {
    ...
    async routeEvent(event: RawBridgeEvent, wallet?: WalletInterface, sessionId?: string): Promise<void> {
        ...
        if (!validation.isValid) {
            console.error('Invalid bridge event:', validation.errors);
            return;
        }
        ...
        try {
            ...
            if (!handler) {
                console.warn('No handler found for event:', event.method);
                return;
            }
    ...
    private notifyTransactionRequestCallbacks(event: EventTransactionRequest): void {
        this.transactionRequestCallbacks.forEach((callback) => {
            try {
                ...
                console.error('Error in transaction request callback:', error);
            }
        });
    }
    ...
    private notifySignDataRequestCallbacks(event: EventSignDataRequest): void {
        this.signDataRequestCallbacks.forEach((callback) => {
            try {
                ...
                console.error('Error in sign data request callback:', error);
            }
        });
    }
