 ...
 */
export class TransactionEmulatedTraceMapper extends Mapper<ToncenterEmulationResponse, TransactionEmulatedTrace> {
    ...
    private mapMetadataEntry(entry: EmulationAddressMetadata): TransactionAddressMetadataEntry {
        return {
            tokenInfo: entry.token_info?.map((info) => this.mapTokenInfo(info)),
        };
    }

    private mapMetadata(metadata: Record<string, EmulationAddressMetadata>): TransactionAddressMetadata {
    ...
    private mapActionDetails(actionType: string, details: EmulationAction['details']): TransactionTraceActionDetails {
