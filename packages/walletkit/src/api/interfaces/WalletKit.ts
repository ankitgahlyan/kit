...
export class WalletKit {
    ...
    private readonly connectionRequestEventMapper = new ConnectionRequestEventMapper();
    private readonly transactionRequestEventMapper = new TransactionRequestEventMapper();
    private readonly signDataRequestEventMapper = new SignDataRequestEventMapper();
    private readonly disconnectionEventMapper = new DisconnectionEventMapper();
    private readonly transactionApprovalResponseMapper = new TransactionApprovalResponseMapper();
    private readonly signDataApprovalResponseMapper = new SignDataApprovalResponseMapper();
     ...
     */
    async handleNewTransaction(wallet: Wallet, request: TransactionRequest): Promise<void> {
        const internalData = this.transactionRequestMapper.map(request);
        return this.internalKit.handleNewTransaction(wallet.getInternalWallet(), internalData);
    }
     ...
     */
    async approveTransactionRequest(event: EventTransactionRequest): Promise<TransactionApprovalResponse> {
     ...
     */
    async signDataRequest(event: EventSignDataRequest): Promise<SignDataApprovalResponse> {
